...
/**
 * A list of related forum posts generated by the Global Search Engine.
 *
 * @param stdClass $discussion The current discussion, to get posts related to first post.
 * @return \core_search\document[]
 */
function mod_forum_get_related_discussions($discussion) {
    if (\core_search\manager::is_global_search_enabled() === false) {
        return false;
    }
    $data = new stdclass();
    $data->q = clean_param($DB->get_field('forum_posts', 'message', array('id' => $discussion->firstpost)), PARAM_TEXT);
    $data->mlt = true;
    $data->areaids = array(\core_search\manager::generate_areaid('mod_forum', 'post'));
    $search = \core_search\manager::instance();
    return $search->paged_search($data, $page);
}
