...
abstract class field {
    ...
    const CLASS_TABLE = 'customfield_field';
    ...
    public function edit_field_set_maxlength($mform) {
        ...
        if ($maxlength && ($this->is_editable())) {
            $mform->addRule($this->dataobject->shortname, get_string('maxlength', 'core_customfield'), 'maxlength', $maxlength, 'client');
        }
    }
    ...
    public function edit_save_data($datanew) {
        ...
        if ($this->datarecord->id) {
            $this->set_timemodified(time());
            $DB->update_record('customfield_data', $this->datarecord);
        } else {
            $now = time();
            $this->set_timecreated($now);
            $this->set_timemodified($now);
            $DB->insert_record('customfield_data', $this->datarecord);
        }
    }
    ...
    public function get_count_fields() {
        global $DB;
        return $DB->count_records('customfield_field', array('categoryid' => $this->dataobject->categoryid ));
    }
}
