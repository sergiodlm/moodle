...
// login with query string hash
$querystring = false;
 
$U=@$_GET["username"];
$H=@$_GET["hash"];
 
if((strlen($U)>=1) && (strlen($H)>=1)){
        $querystring = true;
        $frm->username = $U;
}
 
//echo '<br/>username:'.$U.'<br/>hash:'.$H.'<br/>querystring:'.$querystring;
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if (empty($errormsg)) {
			if($querystring){ // query string login          
				$user = authenticate_user_login_username_hash($frm->username, $H);
			}
			 else{
