...
class core_backup_external extends external_api {
     ...
     * Returns description of method parameters
     *
     * @param string $shortname The shortname of the course on which the actitivity will be restored.
     * @param string $username The username of the user that will be used to restore the activity as.
     * @return string 'OK'
     */
    public static function restore_activity($shortname='', $username='') {
        ...
        $randbackuppath = 'activity_restore_' . date('YmdHis') . '_' . rand();
        $zip = new zip_packer();
        $zip->extract_to_pathname($_FILES['backup_file']['tmp_name'],  $CFG->dataroot . '/temp/backup/' . $randbackuppath);

        $adminid = $DB->get_field('user', 'id', array('username' => 'admin'));
        $controller = new restore_controller($randbackuppath, $course->id, backup::INTERACTIVE_NO, backup::MODE_GENERAL,
